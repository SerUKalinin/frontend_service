...
interface EditObjectModalProps {
  isOpen: boolean;
  onClose: () => void;
  onEdit: (id: number, objectData: any) => Promise<void>;
  object: Object | null;
...
const EditObjectModal: React.FC<EditObjectModalProps> = ({
  ...
  useEffect(() => {
    if (object) {
      setFormData({
        parentId: object.parentId?.toString() || '',
        name: object.name,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onEdit(object.id, formData);
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div>
                    ...
                    >
                      ...
                      {parentObjects
                        .filter(obj => obj.id !== object.id.toString()) // Исключаем текущий объект из списка родителей
                        .map((obj) => (
